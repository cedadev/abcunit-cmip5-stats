"""Contains so simple arg parse functions"""
import argparse
from lib import defaults

def parse_args_stat():
    """arg parse for one argument"""
...
def parse_args_two():
    ...
    parser.add_argument('-m', '--model', nargs=1, type=str, choices=model_choices, required=True,
                        help=f'Institue and model combination to run statistic on, '
                             f'one of: {model_choices}')
    return parser.parse_args()
